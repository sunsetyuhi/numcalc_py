#2次のRunge-Kutta法による常微分方程式の解法プログラム
...
#2次のRunge-Kutta法
def runge_kutta2(func_dydt, t, y, dt):
    k1 = func_dydt(t, y)*dt
    k2 = func_dydt(t+dt, y+k1)*dt
    dy = (k1 +k2)/2.0  #変化量を計算
...
def ode_calc(func_dydt, y_start, t_start, t_end, dt):
    ...
    #ずっと繰り返す
    while(True):
        t, y  = runge_kutta2(func_dydt, t, y, dt)  #変化量を計算
        print("t = {:.7f},  y = {:.7f}".format(t, y))
...
if __name__ == '__main__':
    #2次のRunge-Kutta法でdt離れた点の値を取得（導関数、tの初期値、yの初期値、刻み幅dt）
    t, y = runge_kutta2(func_dydt, 0.0, 0.0, 1e-1)
    print("t = {:.7f},  y = {:.7f}".format(t, y))
